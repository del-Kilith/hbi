...
function updateGradientGuideline(from, to) {
  function drawGradient(c1, c2, row) {
    const grd = _ctx.createLinearGradient(0, row, resolution, 1)
    grd.addColorStop(0, c1)
    grd.addColorStop(1, c2)
    _ctx.fillStyle = grd
    _ctx.fillRect(0, row, resolution, 1)
  }
...
function getColorAtPercent(row, percent) {
  const pixel = Math.floor(percent * resolution)
  const [r, g, b] = _ctx.getImageData(pixel, row, 1, 1).data
  const hex = n => n.toString(16).padStart(2, '0')
  console.log(`Getting data in row ${row} at ${percent}% [pixel: ${pixel}]`)
